...
import { TWeek, TPaymentTypes, TAutoPickStrategy } from '../commonTypes';
...
export const getAllLeagues = new ValidatedMethod({
	...
	run() {
		...
		const leagues = _.chain(users).pluck('leagues').flatten().uniq().value();
...
export const getUsers = new ValidatedMethod({
	...
	run({ activeOnly, league }) {
		...
		activeUsers = User.find(filter).fetch();
...
export const validateReferredBy = new ValidatedMethod({
	...
	run({ referred_by, user_id = this.userId }) {
		const currentUser = User.findOne(user_id);
		const allUsers = User.find({ trusted: true }).fetch();
		let foundUsers;
...
if (dbVersion < 2) {
	UserConditional = Class.create({
		...
		helpers: {
			getSelectedWeek() {
				...
				hrs = differenceInHours(new Date(), new Date(selected_on));
	...
	UserConditional = Class.create({
		...
		helpers: {
			getSelectedWeek() {
				const NO_WEEK_SELECTED = null;
				const { week, selected_on } = this.selected_week;
				let hrs;

...
const Navigation: FC<TNavigationDataProps> = ({
	...
	const seasonStart = nextGame && formatDate(nextGame.kickoff);
	...
	return (
		<div
			className={`col-10 col-sm-3 col-lg-2 ${
				openMenu ? '' : 'd-sm-block'
			...
			{!logoutOnly ? (
				<div className="sidebar-inner">
					<span className="d-sm-none" onClick={toggleMenu}>
						<FontAwesomeIcon className="close-menu" icon="times" />
					</span>
							...
							{currentWeek !== selectedWeek ? (
								<li>
									<a href="#" onClick={goToCurrentWeek}>
										<FontAwesomeIcon icon="reply" fixedWidth /> Current Week
									</a>
								</li>
							) : null}
						</ul>
							...
							<>
								{survivorPicks.length === 17 && (
									<li key="make-survivor-picks">
										<NavLink to="/survivor/set" exact activeClassName="active">
											Make Survivor Picks
										</NavLink>
									</li>
								)}
								{nextGame && (nextGame.week > 1 || nextGame.game > 1) && (
									<li key="view-survivor-picks">
										<NavLink to="/survivor/view" exact activeClassName="active">
											View Survivor Picks
										</NavLink>
									</li>
								)}
					...
					<ul className="nav nav-sidebar flex-column">
						...
						<li>
							<a href="#" onClick={(): void => toggleRightSlider('scoreboard')}>

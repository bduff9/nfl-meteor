...
const ScoreBoard = React.createClass({
  ...
  render: function() {
    ...
    if (this.state.hasPrev) prevArrow = <i className="fa fa-large fa-caret-left" onClick={this._jumpToWeekBefore}></i>
    if (this.state.hasNext) nextArrow = <i className="fa fa-large fa-caret-right" onClick={this._jumpToWeekAhead}></i>
    return (
      <div className="scoreBoard">
        <h3 className="center-block text-center">NFL Scoreboard</h3>
        <div className="innerScoreboard">
          <div className="text-center weekDisp">
            {prevArrow}
  ...
  render: function() {
    ...
    for (var i = 0, iLen = data.length; i < iLen; i++) {
      if (data[i].CALCULA001 !== currDay) {
        ...
        rows.push(
          <tr className="text-center" key={"head" + i} id={"WEEK" + data[i].WEEK}>
            <td colSpan="3"><u>{currDay}</u></td>
          </tr>
        );
      }
      ...
      rows.push(
        <tr key={"v" + i} className={data[i].REDZONE === "V" ? "bg-danger" : ""}>
        <td>{data[i].VTEAM} {data[i].HASPOSS === "V" ? <i className="fa fa-large fa-lemon-o"></i> : ""}</td>
          <td>{data[i].STATUS === 'P' ? '' : data[i].VSCORE}</td>
          <td></td>
        </tr>
      );
      rows.push(
        <tr key={"h" + i} className={data[i].REDZONE === "H" ? "bg-danger" : ""}>
          <td>{data[i].HTEAM} {data[i].HASPOSS === "H" ? <i className="fa fa-large fa-lemon-o"></i> : ""}</td>
          <td>{data[i].STATUS === 'P' ? '' : data[i].HSCORE}</td>
          <td>{statusDisp}</td>
        </tr>
      );
      rows.push(
        <tr className="divider" key={"d" + i}>
          <td colSpan="3"></td>
        </tr>
      );
    }
...
jQuery.ajax({
  ...
  success:function(data) {
    React.render(<ScoreBoard data={data} sync={sync} user={user} />, mountNode);
  },
  dataType:'json'
});
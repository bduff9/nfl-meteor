...
export const getSortedSurvivorPicks = new ValidatedMethod({
	...
	run ({ league }: { league: string }): TSortedSurvivor[] {
		const picks: TSurvivorPick[] = SurvivorPick.find(
			{ league },
			// eslint-disable-next-line @typescript-eslint/camelcase
			{ sort: { user_id: 1, week: 1 } },
		).fetch();
		let pickObjs: TSortedSurvivor[] = [];
		let currPlace = 1;
...
export const setSurvivorPick = new ValidatedMethod({
	...
	run ({ gameId, league, teamId, teamShort, week }) {
		...
		const pick = survivorPicks.filter(pick => pick.week === week)[0];
		const usedIndex = survivorPicks.findIndex(pick => pick.pick_id === teamId);

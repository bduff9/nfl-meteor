...
export const getGamesForWeek = (week: TWeek): any => {
...
export const populateGames = (): void => {
	for (let w: TWeek = 1; w <= WEEKS_IN_SEASON; w++) populateGamesForWeek(w);
};
...
export const updateGames = (): void => {
	...
	games.forEach(
		(gameObj): void => {
			...
			if (hTeamData.spread)
				game.home_spread =
					Math.round(parseFloat(hTeamData.spread, 10) * 10) / 10;

			if (vTeamData.spread)
				game.visitor_spread =
					Math.round(parseFloat(vTeamData.spread, 10) * 10) / 10;
